...
from utils.func import print_state_dict
...
class BaseAgent:
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.cfg.get("experiment", {}).get("wandb", False):
            ...
            wandb_kwargs.setdefault("sync_tensorboard", False) #todo cannot set step when sync_tensorboard=True
        ...
        if self.cfg.get("experiment", {}).get("wandb", False):
            for k, v in self.tracking_data.items():
                if k.endswith("(min)"):
                    ...
                    wandb.log({k: np.min(v)}, step=timestep)
                elif k.endswith("(max)"):
                    # self.writer.add_scalar(k, np.max(v), timestep)
                    wandb.log({k: np.max(v)}, step=timestep)
                elif "Video" in k or "video" in k:
                    video_path = v[-1] if isinstance(v, list) and v else v
                    if video_path and isinstance(video_path, str) and os.path.isfile(video_path):
                        wandb.log({"Video": wandb.Video(video_path, fps=30)}, step=timestep)
                else:
                    # self.writer.add_scalar(k, np.mean(v), timestep)
                    wandb.log({k: np.mean(v)}, step=timestep)
    ...
    def write_checkpoint(self, timestep: int, timesteps: int) -> None:
        ...
        # whole agent
            ...
            print_state_dict(modules, title="Saved checkpoint structure")
        ...
        if self.checkpoint_store_separately:
            for name, module in self.checkpoint_modules.items():
                torch.save(
                    self._get_internal_value(module),
                    os.path.join(self.experiment_dir, "checkpoints", f"last_{name}.pt"),
                )
        else:
            modules = {}
            for name, module in self.checkpoint_modules.items():
                modules[name] = self._get_internal_value(module)
            torch.save(modules, os.path.join(self.experiment_dir, "checkpoints", "last_agent.pt"))

        print("Successfully saved checkpoint to disk")
    ...
    def record_transition(
        ...
        if self.write_interval > 0:
            ...
            if len(self._track_rewards):
                ...
                self.tracking_data["Reward / Episode reward (max)"].append(np.max(track_rewards))
                self.tracking_data["Reward / Episode reward (min)"].append(np.min(track_rewards))
                self.tracking_data["Reward / Episode reward (mean)"].append(np.mean(track_rewards))
            ...
            if 'wandb' in infos and infos['wandb']:
                for k, v in infos['wandb'].items():
                    self.tracking_data[f'{k}'].append(v)
            
        if 'log' in infos and infos['log'] and self.training:
            checkpoints_dir = os.path.join(self.experiment_dir, "checkpoints")
            if os.path.exists(checkpoints_dir):
                for k, v in infos['log'].items():
                    with open(os.path.join(checkpoints_dir, f'{k}.txt'), 'w') as f:
                        f.write(str(v))
    ...
    def load(self, path: str, is_train: bool, skip_modules: list[str] | None=None) -> None:
        ...
        print_state_dict(modules, title="Loaded checkpoint structure")
            ...
            for name, data in modules.items():
                ...
                if module is not None:
                    if hasattr(module, "load_state_dict"):
                        ...
                        if is_train and hasattr(module, "eval"):
                            module.eval()
                    else:
                        raise NotImplementedError
                else:
