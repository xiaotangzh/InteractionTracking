...
class PPO(BaseAgent):
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            self.memory.create_tensor(name="states", size=self.observation_size, dtype=torch.float32)
            self.memory.create_tensor(name="actions", size=self.action_size, dtype=torch.float32)
            ...
            self.rewards = torch.zeros((self.memory.num_envs), dtype=torch.float32, device=self.device)
